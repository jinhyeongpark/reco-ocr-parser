...
### 4) 검토 및 수기 보정 워크플로우 (Human-in-the-Loop)
**한계**: OCR 엔진 특성상 저화질 이미지나 심한 노이즈가 포함된 경우 정규식만으로는 100% 완벽한 추출이 불가능하며, 현재는 결측치 발생 시 `needsReview` 플래그를 통해 '알림'을 주는 단계까지 구현되어 있습니다.

**개선**:
  - **이미지 대조 관리 도구**: 향후 관리자 페이지에서 OCR 원본 이미지와 파싱된 데이터를 나란히 배치하여, `needsReview`가 활성화된 티켓을 즉시 확인하고 교정할 수 있는 UI/UX 도입이 필요합니다.
  - **데이터 정합성 동기화**: 본 프로젝트에서는 이를 대비하여 **'수정(Update) API'**를 선제적으로 구현하였습니다. 관리자가 데이터를 수동으로 교정하면 DB뿐만 아니라 로컬 저장소의 JSON/CSV 파일도 즉시 최신본으로 갱신하여 시스템 전체의 데이터 정합성을 유지하도록 설계했습니다.
